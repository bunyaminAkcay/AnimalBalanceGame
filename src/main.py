...
def main():
    ...
    for i, tag in enumerate(animalTagsInScene):
        ...
        barObjects.append(bar.animalSprite)
        barObjects.append(bar.backgroundSprite)


    resultScreen = ResultScreen(barObjects, (-820, 230), level1)
    level1.addGameObject(resultScreen)

    timer = Timer( "Timer start", 1650, 390, 96, -20, (255,255,255), 5, board, resultScreen)
    level1.addGameObject(timer)

    game = Game(screenSize, "level1", 60, 60, pygame.FULLSCREEN, (34, 32, 52))
    game.run()
